...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const prompt = `You are a LeetCode problem-solving assistant. Your role is to guide users through solving problems in a step-by-step and detailed manner. Always follow this format:
...
6. Always structure the response as valid JSON. Avoid unnecessary formatting or deviation from this structure.`;
    ...
    const result = await chatSession.sendMessage(
      "Generate the LeetCode problem solution based on the provided problem number and language."
    );
    ...
    try {
      ...
      if (
        ...
        !parsedResponse.solution.language ||
        !parsedResponse.solution.code
      ) {
        throw new Error("Invalid response format. Required fields are missing.");
      }
      ...
      return NextResponse.json(
        { error: "Invalid JSON response format. Please try again." },
        { status: 500 }
      );
    }
  } catch (error) {
